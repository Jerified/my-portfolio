...
export const Feedback = () => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'w-fit overflow-hidden border py-2 shadow-sm bg-white/10 backdrop-blur-md border-white/20'
      )}>
      <span className="flex items-center justify-center gap-3 pl-4 pr-2">
        <div className="text-sm text-white">Like my portfolio?</div>
        ...
        <AnimatePresence>
          {!isSubmitted ? (
            <motion.span exit={{ opacity: 0 }} initial={{ opacity: 1 }}>
              <textarea
                ref={textRef}
                placeholder="Your portfolio is awesoooome"
                className="min-h-32 w-full resize-none rounded-md border bg-transparent p-2 text-sm placeholder-neutral-400 focus:border-neutral-400 focus:outline-0 dark:border-neutral-800 focus:dark:border-white text-white"
              />
              <div className="flex h-fit w-full justify-end">
                <button
                  onClick={() => submitFeedback(happiness!, textRef.current!.value || '')}
                  className={cn(
                    'mt-1 flex h-9 items-center justify-center rounded-md border bg-gradient-to-r from-[#d4a574] to-[#c49b6a] px-2 text-sm text-black font-semibold',
                    {
            ...
            <motion.div
              ...
              <motion.div
                variants={item}
                className="flex h-8 min-h-8 w-8 min-w-8 items-center justify-center rounded-full bg-gradient-to-r from-[#d4a574] to-[#c49b6a]">
                <Check strokeWidth={2.5} size={16} className="stroke-black" />
              </motion.div>
              <motion.div variants={item} className="text-white">Your feedback has been received!</motion.div>
              <motion.div variants={item} className="text-white">Thank you for your help.</motion.div>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
